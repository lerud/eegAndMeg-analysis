...
if denoiseMatlab:
    ...
    print(
        f"Set to NOT load tsPCA/SNS denoised dataset output from MATLAB, so loading raw fif"
    )
...
if printAllEvents:
    print(
        np.concatenate((presStartEvents[:, None], presStartEventCodes[:, None]), axis=1)
    )
print("\n")
...
if printAllEvents:
    print(
        np.concatenate((presStopEvents[:, None], presStopEventCodes[:, None]), axis=1)
    )
print("\n")
...
if printAllEvents:
    print(
        np.concatenate((trigStartEvents[:, None], trigStartEventCodes[:, None]), axis=1)
    )
print("\n")
...
if printAllEvents:
    print(
        np.concatenate((trigStopEvents[:, None], trigStopEventCodes[:, None]), axis=1)
    )
print("\n")
...
if doPresentation:
    ...
    if doParallel:
        ...
        results = joblib.Parallel(n_jobs=n_jobs, backend=parallelBackend, verbose=49)(
...
if doPresentation:
    ...
    plt.plot(
        np.linspace(windowStart, windowEnd, freqPres_portion.shape[0]), freqPres_portion
    )
    plt.title(
    ...
    # ws_portion = filtered_ws[int(args.fs_resamp*delay):int(args.fs_resamp*duration)]
    plt.plot(
        np.linspace(windowStart, windowEnd, timePres_portion.shape[0]), timePres_portion
    )
    plt.title(
        "TRFs from regularized time domain deconvolution: Presentation", fontsize=17
    )
    plt.xlabel("Time (s)", fontsize=17)
    plt.ylabel("Potential (nV)", fontsize=17)
    plt.xticks(fontsize=15)
...
if doTriggy:
    ...
    # ws_portion = filtered_ws[int(args.fs_resamp*delay):int(args.fs_resamp*duration)]
    plt.plot(
        np.linspace(windowStart, windowEnd, freqTrig_portion.shape[0]), freqTrig_portion
    )
    plt.title(
        "TRFs from regularized frequency domain deconvolution: Triggy", fontsize=17
    )
    plt.xlabel("Time (s)", fontsize=17)
    plt.ylabel("Potential (nV)", fontsize=17)
    plt.xticks(fontsize=15)
    ...
    # ws_portion = filtered_ws[int(args.fs_resamp*delay):int(args.fs_resamp*duration)]
    plt.plot(
        np.linspace(windowStart, windowEnd, timeTrig_portion.shape[0]), timeTrig_portion
    )
