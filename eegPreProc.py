...
def process_initial_raw_eeg(readpath, writepath, refs=None, freqsToNotch=None):
    ...
    for i in range(10):
        ...
        if i <= 8:
            ...
            sp.io.savemat(f'{str(outpath)}_mat_segment{i}.mat', 
        ...
        # last mod n_segs samples
            print(f"segment {i} saving last {x[:, i * n_samp//n_segs:].shape[1]} samples")
            sp.io.savemat(f'{str(outpath)}_mat_segment{i}.mat', 
...
try:  # Try to load the raw fif file, which will work if we've made it before
    process_initial_raw_eeg(mainDir+subject+subDirs+subject+'_'+runName+'.fif', '/Users/karl/map/'+subject+subDirs, refs, freqsToNotch)
except:  # if it doesn't exist, we'll make it first, and then load it
    targetFiles=sorted(glob.glob(mainDir+subject+subDirs+subject+'_*'+runName+'*.bdf'))
    if len(targetFiles)==1:
        ...
        print(f'There are 2 bdf files being concatenated in this order: {targetFiles[0]} and {targetFiles[1]}')
        raw=mne.io.read_raw_bdf(targetFiles[0])
